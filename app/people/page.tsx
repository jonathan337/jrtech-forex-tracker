        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 items-stretch">
          {filteredPeople.map((person) => {
            ...
            <Card key={person.id} className="shadow-md hover:shadow-lg transition-shadow min-w-0 overflow-hidden flex flex-col h-full">
              <CardHeader className="border-b border-[#eef0f3] border-b space-y-3">
                <div className="flex gap-3 min-w-0">
                  <Link
                    href={`/people/${person.id}?year=${budgetYear}&month=${budgetMonth}`}
                    className="w-12 h-12 rounded-full bg-indigo-600 flex items-center justify-center shrink-0 text-white font-bold text-lg hover:ring-2 hover:ring-indigo-200 hover:ring-offset-2 transition-shadow"
                  ...
                  <div className="min-w-0 flex-1">
                    <CardTitle className="text-lg font-semibold leading-snug text-gray-900 break-words hyphens-auto">
                      <Link
                        href={`/people/${person.id}?year=${budgetYear}&month=${budgetMonth}`}
                        className="hover:text-indigo-900 hover:underline decoration-indigo-300 underline-offset-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300 rounded-sm"
                      >
                        {person.name}
                      </Link>
                    </CardTitle>
                ...
                <div
                  className="flex flex-wrap items-center gap-2 pt-2 border-t border-indigo-100/60 -mb-1"
                  role="toolbar"
                ...
                >
                  <Button
                    ...
                    className={
                      addPaymentForPersonId === person.id
                        ? 'bg-indigo-700 hover:bg-indigo-800 text-white shrink-0'
                  ...
                  <span className="hidden sm:block w-px h-6 bg-indigo-100/90 shrink-0" aria-hidden />
                  <Button
                    variant="ghost"
                    size="sm"
