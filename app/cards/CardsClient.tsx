...
export function CardsClient({
  ...
  const [cards, setCards] = useState<CardType[]>(initialData?.cards ?? [])
  ...
  const [defaultExchangeRate, setDefaultExchangeRate] = useState<number | null>(
    initialData?.defaultExchangeRate ?? null
  )
  const [banks, setBanks] = useState<Bank[]>(initialData?.banks ?? [])
  // SSR already delivered cards/people/settings; skip the first client load().
  ...
  useEffect(() => {
    ...
    // First authenticated render already has SSR data — don't refetch it.
