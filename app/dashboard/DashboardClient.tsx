...
type DashboardInitialData = {
  ...
  totalOwedToPeopleTTD: number
  usdCostSummary: MonthUsdCostSummary | null
}
...
export function DashboardClient({
  ...
  // Seed the month from the server so the SSR data matches what we show; day 1
  // keeps the prev/next-month buttons off the end-of-month rollover bug.
  ...
  const [exchangeRate, setExchangeRate] = useState<ExchangeRate | null>(() =>
    initialData &&
    typeof initialData.defaultExchangeRate === 'number' &&
    Number.isFinite(initialData.defaultExchangeRate)
      ? {
          selling: initialData.defaultExchangeRate,
          buying: initialData.defaultExchangeRate,
  ...
  const didInitialLoadRef = useRef(initialData != null)
  // SSR already delivered the current month, so skip the client's first fetch of it.
  ...
  const [totalOwedToPeopleTTD, setTotalOwedToPeopleTTD] = useState(
    initialData?.totalOwedToPeopleTTD ?? 0
  )
  const [usdCostSummary, setUsdCostSummary] = useState<MonthUsdCostSummary | null>(
    initialData?.usdCostSummary ?? null
  )
  ...
  useEffect(() => {
    ...
    // First authenticated render already has the SSR data for this month — don't
    // immediately refetch it; just clear the loading flag.
  ...
  return (
    <div className="space-y-6 min-w-0">
      ...
      {usdCostSummary && (
        <Card className="shadow-md bg-gradient-to-r from-emerald-50/50 to-teal-50/50 min-w-0 overflow-hidden">
          <CardHeader className="pb-3">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between min-w-0">
              <div className="min-w-0">
