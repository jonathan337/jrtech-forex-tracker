...
export function PaymentsClient({
  ...
  initialData: PaymentsInitialData | null
  initialYear: number
  initialMonth: number
}) {
  ...
  // SSR already delivered this month's payments, so skip the client's first fetch of them.
  ...
  useEffect(() => {
    ...
    // People come from a separate endpoint the SSR payload doesn't include, so
    // always load them (including on the very first authenticated render).
    void fetchPeople()
    // First authenticated render already has the SSR payments for this month —
    // don't immediately refetch them; just clear the loading flag.
