...
export function AvailabilityClient({
  ...
  // Only the very first cold load shows the full-page spinner.
  const didInitialLoadRef = useRef(initialData != null)
  // SSR already delivered the availability list; skip the first client fetch of it.
  ...
  useEffect(() => {
    ...
    const load = async () => {
      // SSR seeded the availability list already; skip only that fetch on the
      // first run. Cards and settings are not part of the SSR payload, so they
      // must still be fetched here to keep the form working.
      const skipAvailability = skipInitialFetchRef.current
      if (skipAvailability) skipInitialFetchRef.current = false
